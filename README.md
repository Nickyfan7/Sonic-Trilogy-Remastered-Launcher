# Sonic-Trilogy-Remastered-Launcher

![Screenshot 2021-04-02 at 5 08 54 PM](https://user-images.githubusercontent.com/65929380/106548954-f7085300-64dd-11eb-9fec-bc63ca61f442.png)
![Screenshot 2021-04-02 at 5 08 54 PM](https://user-images.githubusercontent.com/65929380/106548952-f66fbc80-64dd-11eb-9ea6-36dfe0fe6224.png)</br>
This is a Launcher intended to run Sonic  1,2,CD,and 3 (2011,2013,2019) it intends to go further however by including:



=== Extras Menu ===
<br>![Screenshot 2021-04-02 at 5 08 54 PM](https://user-images.githubusercontent.com/65929380/106548955-f7085300-64dd-11eb-926d-3e8d109c8752.png)
![Screenshot 2021-04-02 at 5 08 54 PM](https://user-images.githubusercontent.com/65929380/106548956-f7085300-64dd-11eb-9eb5-550bf14e1016.png)

Here you can read up on info for the Games such as their Release Dates,Plots, Prototypes, Cheat Codes, and Interviews.
You can also view Promotional Artwork, the Games Manuals, and Unused Content.

=== Controller Support ===
You'll be able to use Xbox Controllers, PS4 Controllers, and Custom Controllers! (Although for those you'll need to set them up.)
this way you get to feel like you're playing on console with a menu to select the games from.

===Console/Mobile Support==
I plan to eventually bring these launchers to Console (Whatever console you can play all three on), and mobile if possible that way you can select the games get info on them, 
launch the protos and more in style!


There are some other plans in store i'm not so sure about implementing but potential plans include:
-Ability to Launch Prototypes, from their respective screen, given you provide it.
-Compatability with the originals (Possible, but not likely.)

